...
"""
description: 通过解析网站制作tk窗口，实现视频播放、下载界面
...
class APP:
    """爱奇艺、优酷等实现在线观看以及视频下载"""
    def __init__(self, width=500, height=300):  # width:主界面宽  height:高
        self.w = width
        self.h = height
        ...
        rb1 = tk.Radiobutton(frame_1, text='通道一', variable=self.v, value=1, width=10, height=3)
        rb2 = tk.Radiobutton(frame_1, text='通道二', variable=self.v, value=2, width=10, height=3)
        ...
        # download = tk.Button(frame_2, text = "下载", font = ('楷体',12),
        #                      fg = 'Purple', width = 2, height = 1, command = self.download_wmxz)
    ...
    def QR_Code(self):
        ...
        if re.match(r'^https?:/{2}\w.+$', self.url.get()):
            ...
            images_pwd = os.path.join(os.getcwd(), 'Images')
            png_path = os.path.join(images_pwd, 'bg.png')
            qr_path = os.path.join(images_pwd, 'qrcode.png')

            run(words=url, picture=png_path, save_name='qrcode.png', save_dir=images_pwd)
