...
def main():
    ...
    def sendMsg():
        """发送消息"""
        str_msg = 'i_chaoren:' + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + '\n '
        txtMsgList.insert(END, str_msg, 'greencolor')  # 插入到tag位置
    ...
    def txtlist():  # 字典实现--书名与内容的对应
        book = {
            'NO.1 《平凡的世界》  路遥': '1.一个平平常常的日子，细蒙蒙的雨丝夹着一星半点的雪花，正纷纷淋淋的向大地飘洒着，时令已快到惊蛰，雪当然再不会存留，往往还没等落地，就消失的无踪无影了，黄土高原严寒而漫长的冬天看来就要过去，但那真正温暖的春天还远远没有到来。\n2、这时候，他也体验到了类似孙少平的那种感觉：只有繁重的体力劳动才使精神上的痛苦变为某种麻木，以致思维局限在机械性活动中。\n3、哭，笑，都是因为欢乐，哭的人知道而笑的人不知道，这欢乐是多少痛苦所换来的。',
    ...
    txtText.insert(END,
                   '1.一个平平常常的日子，细蒙蒙的雨丝夹着一星半点的雪花，正纷纷淋淋的向大地飘洒着，时令已快到惊蛰，雪当然再不会存留，往往还没等落地，就消失的无踪无影了，黄土高原严寒而漫长的冬天看来就要过去，但那真正温暖的春天还远远没有到来。\n2、这时候，他也体验到了类似孙少平的那种感觉：只有繁重的体力劳动才使精神上的痛苦变为某种麻木，以致思维局限在机械性活动中。\n3、哭，笑，都是因为欢乐，哭的人知道而笑的人不知道，这欢乐是多少痛苦所换来的。')
    # insert(插入位置，插入内容)
    # 2.Button控件
    ...
    Linkman = ['曹操', '刘备', '孙权', '关羽', '张飞', '赵云', '马超', '黄忠', '张郃', '姜维', '夏侯惇', '魏延', '张辽', '周瑜', '贾诩', '典韦', '吕布',
               '袁绍', '袁术', '貂蝉', '董卓', '华佗', '诸葛亮', '郭嘉', '孙策', '孙坚', '太史慈', '鲁肃', '黄盖', '程普', '程昱', '司马懿', '曹丕', '曹植',
               '曹睿']
    for line in Linkman:
