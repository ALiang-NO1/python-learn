...
class MyApp(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.history = tk.Text(self, bg='silver', height=50)
        self.history.grid(row=2, columnspan=3)

        self.quit = tk.Button(self, text="退出", fg="red", command=self.master.destroy)
        self.quit.grid(row=3, column=1, columnspan=1)

        self.clear = tk.Button(self, text="清屏", fg="blue", command=self.clear)
        self.clear.grid(row=3, column=0, columnspan=1)

    def show_msg(self):
        self.msg = tk.Label(self, text=f"你答对了 {self.right} 道题（共 {self.counter} 道）")
        self.msg.grid(row=1, columnspan=3)
